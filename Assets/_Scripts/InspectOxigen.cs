...
public class InspectOxigen : MonoBehaviour
{
    bool inRange = false;
    ...
    void Start()
    {
        ...
        TasksScript.tasksScriptInstance.RobotInspectOxigenPosition = robotLocation.transform.position;
        TasksScript.tasksScriptInstance.RobotInspectOxigenRotation = robotLocation.transform.rotation;
    }
    ...
    void Update()
    {
        ...
        if (TasksScript.tasksScriptInstance.robotInspectOxigen)
        {
            firstButton.transform.position = firstButtonPushed.transform.position;
            secondButton.transform.position = secondButtonPushed.transform.position;
            thirdButton.transform.position = thirdButtonPushed.transform.position;

            TasksScript.tasksScriptInstance.oxigenInspectionDone = true;
        }
        ...
        if (inRange)
        {
            ...
            if (Input.GetKey(KeyCode.E) && Vector3.Distance(selectedButton.transform.position, selectedButtonPushed.transform.position) > 0.025f)
            {
                ...
                if (buttonsPushed == 3)
                {
                    TasksScript.tasksScriptInstance.oxigenInspectionDone = true;
                    buttonsPushed = 0;
                    TasksScript.tasksScriptInstance.inspectTask3.text = "Inspection Done";
                    Debug.Log("task inspect:" + TasksScript.tasksScriptInstance.oxigenInspectionDone);
                }
            }
        }
